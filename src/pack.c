...
;// gles_glPixelStoref (pname, param);
...
void glGetIntegerv (GLenum pname, GLint *data){    LOAD_GLES    
 gles_glGetIntegerv (pname, data);

}

const GLubyte *glGetString (GLenum name){    LOAD_GLES    
 if(name == GL_RENDERER){
  return (const GLubyte *)"vgpu 1.0";
 }
 if(name == GL_VERSION){
...
;// gles_glCopyTexSubImage1D (target, level, xoffset, x, y, width);
...
;// gles_glTexSubImage1D (target, level, xoffset, width, format, type, pixels);
...
;// gles_glCompressedTexImage1D (target, level, internalformat, width, border, imageSize, data);
...
;// gles_glCompressedTexSubImage1D (target, level, xoffset, width, format, imageSize, data);
...
;// gles_glMultiDrawArrays (mode, first, count, drawcount);
...
;// gles_glMultiDrawElements (mode, count, type, indices, drawcount);
...
void *glMapBuffer (GLenum target, GLenum access){    LOAD_GLES    
 return NULL;// gles_glMapBuffer (target, access);
...
;// gles_glGetVertexAttribdv (index, pname, params);
