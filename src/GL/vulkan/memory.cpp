...
Memory::Release(void)
{
    ...
    if(mVkMemory != VK_NULL_HANDLE) {
        vk_FreeMemory(mVkContext->vkDevice, mVkMemory, NULL);
...
Memory::GetData(VkDeviceSize size, VkDeviceSize offset, void *data) const
{
    ...
    void *pData;
    VkResult err = vk_MapMemory(mVkContext->vkDevice, mVkMemory, offset, size, mVkMemoryFlags, &pData);
    assert(!err);

    if(err != VK_ERROR_OUT_OF_HOST_MEMORY && err != VK_ERROR_OUT_OF_DEVICE_MEMORY && err != VK_ERROR_MEMORY_MAP_FAILED)
    {
        memcpy(data, pData, size);
        vk_UnmapMemory(mVkContext->vkDevice, mVkMemory);
...
Memory::SetData(VkFormat srcFormat, bool normalize, VkDeviceSize size, VkDeviceSize offset, const void *data)
{
    ...
    VkResult err = vk_MapMemory(mVkContext->vkDevice, mVkMemory, offset, size ? size : mVkRequirements.size, mVkMemoryFlags, &pData);
    assert(!err);
...
Memory::GetBufferMemoryRequirements(VkBuffer &buffer)
{
    ...
    vk_GetBufferMemoryRequirements(mVkContext->vkDevice, buffer, &mVkRequirements);
...
Memory::GetImageMemoryRequirements(VkImage &image)
{
    ...
    memset((void *)&mVkRequirements, 0, sizeof(mVkRequirements));
    vk_GetImageMemoryRequirements(mVkContext->vkDevice, image, &mVkRequirements);
}
...
Memory::BindBufferMemory(VkBuffer &buffer)
{
    ...
    VkResult err = vk_BindBufferMemory(mVkContext->vkDevice, buffer, mVkMemory, 0);
    assert(!err);
...
Memory::BindImageMemory(VkImage &image)
{
    ...
    VkResult err = vk_BindImageMemory(mVkContext->vkDevice, image, mVkMemory, 0);
    assert(!err);
...
Memory::Allocate()
{
    ...
    assert(!err);
    err = vk_AllocateMemory(mVkContext->vkDevice, &allocInfo, NULL, &mVkMemory);
    assert(!err);
