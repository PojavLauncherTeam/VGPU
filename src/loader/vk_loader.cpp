...
int InitVulkan(void) {
    ...
    eglChooseConfig(eglDisplay, configAttribs, pbufConfigs, 1, &configsFound);
    ...
    eglContext = eglCreateContext(eglDisplay, pbufConfigs[0], EGL_NO_CONTEXT, egl_context_attrib);
    if(!eglContext) {
        return 0;
    }
    
    eglSurface = eglCreatePbufferSurface(eglDisplay, pbufConfigs[0], egl_attribs);
    if(!eglSurface) {
