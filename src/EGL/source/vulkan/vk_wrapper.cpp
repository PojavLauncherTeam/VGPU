...
static const char* kTAG = "VGPU_VK: ";
#define LOGI(...) \
...
#define CALL_VK(func) \
	if (VK_SUCCESS != (func)) \
	{ printf("Call failed: " #func " File[%s], line[%d]", __FILE__, __LINE__); }
...
void CreateInstance(VkInstance *instance) {
  ...
  VkApplicationInfo appInfo{};
    ...
    appInfo.pApplicationName = "VGPU_VK";
  ...
  CALL_VK(vk_CreateInstance(&instanceCreateInfo, nullptr, instance));
  
}

void CreateAndroidSurface(VkInstance instance, ANativeWindow* platformWindow, VkSurfaceKHR* surface) {
  ...
  CALL_VK(vk_CreateAndroidSurfaceKHR(instance, &createInfo, nullptr, surface));
...
void CreateVulkanDevice(VkInstance instance, ANativeWindow* platformWindow, VulkanDeviceInfo* device_) {
  instance_extensions.push_back("VK_KHR_surface");
  instance_extensions.push_back("VK_KHR_android_surface");
  ...
  uint32_t gpuCount = 0;
  CALL_VK(vk_EnumeratePhysicalDevices(instance, &gpuCount, nullptr));
  VkPhysicalDevice tmpGpus[gpuCount];
  CALL_VK(vk_EnumeratePhysicalDevices(instance, &gpuCount, tmpGpus));
  device_->gpuDevice_ = tmpGpus[0];  // Pick up the first GPU Device

  vk_GetPhysicalDeviceMemoryProperties(device_->gpuDevice_,
                                      &device_->gpuMemoryProperties_);

  // Find a GFX queue family
  uint32_t queueFamilyCount;
  vk_GetPhysicalDeviceQueueFamilyProperties(device_->gpuDevice_, &queueFamilyCount,
                                           nullptr);
  ...
  device_->queueFamilyIndex_ = queueFamilyIndex;
  ...
  VkDeviceQueueCreateInfo queueCreateInfo{
      ...
      .pNext = nullptr,
      .flags = 0,
      .queueFamilyIndex = device_->queueFamilyIndex_,
      .queueCount = 1,
  ...
  CALL_VK(vk_CreateDevice(device_->gpuDevice_, &deviceCreateInfo, nullptr,
                         &device_->device_));
  vk_GetDeviceQueue(device_->device_, 0, 0, &device_->queue_);
}
...
void DestroyInstance(VkInstance instance) {
	vk_DestroyInstance(instance, nullptr);
}

void DestroySurfaceKHR(VkInstance instance, VkSurfaceKHR surface) {
	vk_DestroySurfaceKHR(instance, surface, nullptr);
}



void CreateSwapChain(VulkanSwapchainInfo* swapchain_, VulkanDeviceInfo* device_) {
  ...
  vk_GetPhysicalDeviceSurfaceCapabilitiesKHR(device_->gpuDevice_, device_->surface_,
                                            &surfaceCapabilities);
  // Query the list of supported surface format and choose one we like
  uint32_t formatCount = 0;
  vk_GetPhysicalDeviceSurfaceFormatsKHR(device_->gpuDevice_, device_->surface_,
                                       &formatCount, nullptr);
  ...
  swapchain_->displaySize_ = surfaceCapabilities.currentExtent;
  swapchain_->displayFormat_ = formats[chosenFormat].format;
  ...
  VkSwapchainCreateInfoKHR swapchainCreateInfo{
      ...
      .pQueueFamilyIndices = &device_->queueFamilyIndex_,
      .preTransform = VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR,
      .presentMode = VK_PRESENT_MODE_FIFO_KHR,
      .clipped = VK_FALSE,
      .oldSwapchain = VK_NULL_HANDLE,
  };
  CALL_VK(vk_CreateSwapchainKHR(device_->device_, &swapchainCreateInfo, nullptr,
                               &swapchain_->swapchain_));

  // Get the length of the created swap chain
  CALL_VK(vk_GetSwapchainImagesKHR(device_->device_, swapchain_->swapchain_,
                                  &swapchain_->swapchainLength_, nullptr));
  delete[] formats;
  LOGI("<-createSwapChain");
}

void DeleteSwapChain(VulkanSwapchainInfo* swapchain_, VulkanDeviceInfo* device_) {
  for (int i = 0; i < swapchain_->swapchainLength_; i++) {
    vk_DestroyFramebuffer(device_->device_, swapchain_->framebuffers_[i], nullptr);
    vk_DestroyImageView(device_->device_, swapchain_->displayViews_[i], nullptr);
  }
  delete[] swapchain_->framebuffers_;
  delete[] swapchain_->displayViews_;
  delete[] swapchain_->displayImages_;

  vk_DestroySwapchainKHR(device_->device_, swapchain_->swapchain_, nullptr);
}
...
void CreateFrameBuffers(VkRenderPass& renderPass,
  ...
  CALL_VK(vk_GetSwapchainImagesKHR(device.device_, swapchain.swapchain_,
  ...
  for (uint32_t i = 0; i < SwapchainImagesCount; i++) {
    ...
    CALL_VK(vk_CreateImageView(device.device_, &viewCreateInfo, nullptr,
  ...
  for (uint32_t i = 0; i < swapchain.swapchainLength_; i++) {
    ...
    CALL_VK(vk_CreateFramebuffer(device.device_, &fbCreateInfo, nullptr,
