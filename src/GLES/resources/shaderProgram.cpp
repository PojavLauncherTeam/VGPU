...
ShaderProgram::CreateVkPipelineCache(const void *initialData, size_t initialDataSize)
{
    ...
    VkResult err = vk_CreatePipelineCache(mVkContext->vkDevice, &info, NULL, &mVkPipelineCache);
    assert(!err);
...
ShaderProgram::UsePrecompiledBinary(const void *binary, size_t binarySize)
{
    ...
    if(mVkPipelineCache != VK_NULL_HANDLE) {
        vk_DestroyPipelineCache(mVkContext->vkDevice, mVkPipelineCache, nullptr);
        mVkPipelineCache = VK_NULL_HANDLE;
    }
...
ShaderProgram::GetBinaryData(void *binary, GLsizei *binarySize)
{
    ...
    if(mVkPipelineCache) {
        vk_GetPipelineCacheData(mVkContext->vkDevice, mVkPipelineCache, &vulkanDataSize, reinterpret_cast<void *>(vulkanDataPtr));
...
ShaderProgram::GetBinaryLength(void)
{
    ...
    if(mVkPipelineCache) {
        vk_GetPipelineCacheData(mVkContext->vkDevice, mVkPipelineCache, &vkPipelineCacheDataLength, nullptr);
    }
...
ShaderProgram::ReleaseVkObjects(void)
{
    ...
    if(mVkDescSet != VK_NULL_HANDLE) {
        vk_FreeDescriptorSets(mVkContext->vkDevice, mVkDescPool, 1, &mVkDescSet);
        mVkDescSet = VK_NULL_HANDLE;
    }
...
ShaderProgram::CreateDescriptorSetLayout(uint32_t nLiveUniformBlocks)
{
    ...
    if( vk_CreateDescriptorSetLayout(mVkContext->vkDevice, &descLayoutInfo, 0, &mVkDescSetLayout) != VK_SUCCESS) {
        assert(0);
        return false;
    }
    ...
    if( vk_CreatePipelineLayout(mVkContext->vkDevice, &pipelineLayoutCreateInfo, 0, &mVkPipelineLayout) != VK_SUCCESS) {
        assert(0);
        return false;
    }
...
ShaderProgram::CreateDescriptorPool(uint32_t nLiveUniformBlocks)
{
    ...
    if( vk_CreateDescriptorPool(mVkContext->vkDevice, &descriptorPoolInfo, 0, &mVkDescPool) != VK_SUCCESS) {
        assert(0);
        return false;
    }
...
ShaderProgram::CreateDescriptorSet(void)
{
    ...
    if( vk_AllocateDescriptorSets(mVkContext->vkDevice, &descAllocInfo, &mVkDescSet) != VK_SUCCESS) {
        assert(0);
        return false;
    }
...
void ShaderProgram::UpdateSamplerDescriptors()
{
    ...
    vk_UpdateDescriptorSets(mVkContext->vkDevice, nLiveUniformBlocks, writes, 0, NULL);
