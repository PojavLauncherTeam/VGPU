...
Context::Clear(GLbitfield mask)
{
    ...
    BeginRendering();
      vk_CmdClearAttachments(mVkContext->mCommandBufferManager->GetActiveCommandBuffer(),
...
void Context::BindUniformDescriptors(VkCommandBuffer *CmdBuffer)
{
    ...
    if(*mStateManager.GetActiveShaderProgram()->GetVkDescSet()) {
        ...
        mStateManager.GetActiveShaderProgram()->UpdateDescriptorSet();
          vk_CmdBindDescriptorSets(*CmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mStateManager.GetActiveShaderProgram()->GetVkPipelineLayout(), 0, 1, mStateManager.GetActiveShaderProgram()->GetVkDescSet(), 0, NULL);
    }
}
...
void Context::BindVertexBuffers(VkCommandBuffer *CmdBuffer, const void *indices, GLenum type, bool indexed, uint32_t vertCount)
{
    ...
    if(mStateManager.GetActiveShaderProgram()->GetActiveVertexVkBuffersCount()) {
        ...
        memset(offsets, 0, sizeof(VkDeviceSize) * mStateManager.GetActiveShaderProgram()->GetActiveVertexVkBuffersCount());
          vk_CmdBindVertexBuffers(*CmdBuffer, 0, mStateManager.GetActiveShaderProgram()->GetActiveVertexVkBuffersCount(), mStateManager.GetActiveShaderProgram()->GetActiveVertexVkBuffers(), offsets);
    }
    ...
    if(validatedBuffer) {
          vk_CmdBindIndexBuffer(*CmdBuffer, ibo->GetVkBuffer(), offset, type == GL_UNSIGNED_INT ? VK_INDEX_TYPE_UINT32 : VK_INDEX_TYPE_UINT16);
    }
}
...
void Context::DrawGeometry(VkCommandBuffer *CmdBuffer, bool indexed, uint32_t firstVertex, uint32_t vertCount)
{
    ...
    if(indexed == false) {
          vk_CmdDraw(*CmdBuffer, vertCount, 1, firstVertex, 0);
    } else {
          vk_CmdDrawIndexed(*CmdBuffer, vertCount, 1, 0, 0, 0);
    }
}
