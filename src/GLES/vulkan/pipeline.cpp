...
Pipeline::Destroy()
{
    ...
    if(mVkPipeline != VK_NULL_HANDLE) {
        vk_DestroyPipeline(mVkContext->vkDevice, mVkPipeline, NULL);
...
Pipeline::UpdateDynamicState(VkCommandBuffer *CmdBuffer, float lineWidth)
{
    ...
    vk_CmdSetViewport  (*CmdBuffer, 0, mVkPipelineViewportState.viewportCount, &mVkViewport);
    vk_CmdSetScissor   (*CmdBuffer, 0, mVkPipelineViewportState.scissorCount , &mVkScissorRect);
    vk_CmdSetLineWidth (*CmdBuffer, lineWidth);
}
...
Pipeline::CreateGraphicsPipeline(void)
{
    ...
    VkResult err = vk_CreateGraphicsPipelines(mVkContext->vkDevice, mVkPipelineCache, 1, &mVkPipelineInfo, NULL, &mVkPipeline);
    assert(!err);
