...
InitVkLayers(uint32_t* nLayers)
{
    ...
    do {
        res = vk_EnumerateInstanceLayerProperties(&layerCount, NULL);
        ...
        res = vk_EnumerateInstanceLayerProperties(&layerCount, vkLayerProperties);
...
CheckVkInstanceExtensions(void)
{
    ...
    do {
        res = vk_EnumerateInstanceExtensionProperties(NULL, &extensionCount, NULL);
        ...
        res = vk_EnumerateInstanceExtensionProperties(NULL, &extensionCount, vkExtensionProperties);
    } while(res == VK_INCOMPLETE);
...
CheckVkDeviceExtensions(void)
{
    ...
    do {
        res = vk_EnumerateDeviceExtensionProperties(GloveVkContext.vkGpus[0], NULL, &extensionCount, NULL);
        ...
        res = vk_EnumerateDeviceExtensionProperties(GloveVkContext.vkGpus[0], NULL, &extensionCount, vkExtensionProperties);
    } while(res == VK_INCOMPLETE);
...
CreateVkInstance(void)
{
    ...
    VkResult err = vk_CreateInstance(&instanceInfo, NULL, &GloveVkContext.vkInstance);
    assert(!err);
...
EnumerateVkGpus(void)
{
    ...
    VkResult err;
    err = vk_EnumeratePhysicalDevices(GloveVkContext.vkInstance, &gpuCount, NULL);
    assert(!err);
    ...
    err = vk_EnumeratePhysicalDevices(GloveVkContext.vkInstance, &gpuCount, GloveVkContext.vkGpus.data());
    assert(!err);
    ...
    vk_GetPhysicalDeviceMemoryProperties(GloveVkContext.vkGpus[0], &GloveVkContext.vkDeviceMemoryProperties);
...
InitVkQueueFamilyIndex(void)
{
    ...
    vk_GetPhysicalDeviceQueueFamilyProperties(GloveVkContext.vkGpus[0], &queueFamilyCount, NULL);
    ...
    vk_GetPhysicalDeviceQueueFamilyProperties(GloveVkContext.vkGpus[0], &queueFamilyCount, queueProperties);
...
CreateVkDevice(void)
{
    ...
    VkResult err = vk_CreateDevice(GloveVkContext.vkGpus[0], &deviceInfo, NULL, &GloveVkContext.vkDevice);
    assert(!err);
...
CreateVkSemaphores(void)
{
    ...
    err = vk_CreateSemaphore(GloveVkContext.vkDevice, &semaphoreCreateInfo, NULL, &GloveVkContext.vkSyncItems->vkDrawSemaphore);
    assert(!err);
    ...
    err = vk_CreateSemaphore(GloveVkContext.vkDevice, &semaphoreCreateInfo, NULL, &GloveVkContext.vkSyncItems->vkAcquireSemaphore);
    assert(!err);
    ...
    err = vk_CreateSemaphore(GloveVkContext.vkDevice, &semaphoreCreateInfo, NULL, &GloveVkContext.vkSyncItems->vkAuxSemaphore);
    assert(!err);
...
InitVkQueue(void)
{
    ...
    vk_GetDeviceQueue(GloveVkContext.vkDevice,
...
TerminateContext()
{
    ...
    if(GloveVkContext.vkSyncItems->vkAcquireSemaphore != VK_NULL_HANDLE) {
        vk_DestroySemaphore(GloveVkContext.vkDevice, GloveVkContext.vkSyncItems->vkAcquireSemaphore, NULL);
        GloveVkContext.vkSyncItems->vkAcquireSemaphore = VK_NULL_HANDLE;
    }

    if(GloveVkContext.vkSyncItems->vkDrawSemaphore != VK_NULL_HANDLE) {
        vk_DestroySemaphore(GloveVkContext.vkDevice, GloveVkContext.vkSyncItems->vkDrawSemaphore, NULL);
        GloveVkContext.vkSyncItems->vkDrawSemaphore = VK_NULL_HANDLE;
    }

    if(GloveVkContext.vkSyncItems->vkAuxSemaphore != VK_NULL_HANDLE) {
        vk_DestroySemaphore(GloveVkContext.vkDevice, GloveVkContext.vkSyncItems->vkAuxSemaphore, NULL);
        GloveVkContext.vkSyncItems->vkAuxSemaphore = VK_NULL_HANDLE;
    }

    if(GloveVkContext.vkDevice != VK_NULL_HANDLE ) {
        vk_DeviceWaitIdle(GloveVkContext.vkDevice);
        vk_DestroyDevice(GloveVkContext.vkDevice, NULL);
        vk_DestroyInstance(GloveVkContext.vkInstance, NULL);
    }
