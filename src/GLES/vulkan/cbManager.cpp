...
CommandBufferManager::~CommandBufferManager()
{
    ...
    if(mVkContext->vkDevice != VK_NULL_HANDLE ) {
        ...
        vk_DeviceWaitIdle(mVkContext->vkDevice);

        for(uint32_t i = 0; i < mVkCommandBuffers.fence.size(); ++i) {
            if(mVkCommandBuffers.fence[i] != VK_NULL_HANDLE) {
                vk_DestroyFence(mVkContext->vkDevice, mVkCommandBuffers.fence[i], NULL);
            }
        }

        if(mVkAuxFence != VK_NULL_HANDLE) {
            vk_DestroyFence(mVkContext->vkDevice, mVkAuxFence, NULL);
        }

        vk_FreeCommandBuffers(mVkContext->vkDevice, mVkCmdPool, mVkCommandBuffers.commandBuffer.size(), mVkCommandBuffers.commandBuffer.data());
        ...
        if(mVkAuxCommandBuffer != VK_NULL_HANDLE) {
            vk_FreeCommandBuffers(mVkContext->vkDevice, mVkCmdPool, 1, &mVkAuxCommandBuffer);
            mVkAuxCommandBuffer = VK_NULL_HANDLE;
        }

        if(mVkCmdPool != VK_NULL_HANDLE) {
            vk_DestroyCommandPool(mVkContext->vkDevice, mVkCmdPool, NULL);
            mVkCmdPool = VK_NULL_HANDLE;
        }
    }
}
...
CommandBufferManager::AllocateVkCmdBuffers(void)
{
    ...
    err = vk_CreateCommandPool(mVkContext->vkDevice, &cmdPoolInfo, NULL, &mVkCmdPool);
    assert(!err);
    ...
    err = vk_AllocateCommandBuffers(mVkContext->vkDevice, &cmdAllocInfo, mVkCommandBuffers.commandBuffer.data());
    assert(!err);
    ...
    err = vk_AllocateCommandBuffers(mVkContext->vkDevice, &cmdAllocInfo, &mVkAuxCommandBuffer);
    assert(!err);
    ...
    for(uint32_t i = 0; i < GLOVE_NUM_VK_COMMAND_BUFFERS; ++i) {
        ...
        err = vk_CreateFence(mVkContext->vkDevice, &fenceInfo, NULL, &mVkCommandBuffers.fence[i]);
        assert(!err);
    ...
    err = vk_CreateFence(mVkContext->vkDevice, &fenceInfo, NULL, &mVkAuxFence);
    assert(!err);
    ...
    for(std::vector<resourceBase_t *>::iterator it = mReferencedResources.begin(); it != mReferencedResources.end();) {
        ...
        if(!resourceBase->mRefCount) {
            ...
            case RESOURCE_TYPE_SHADER: {
                ...
                vk_DestroyShaderModule(mVkContext->vkDevice, resource->mResourcePtr, NULL);
                } break;
            case RESOURCE_TYPE_PIPELINE_LAYOUT: {
                ...
                vk_DestroyPipelineLayout(mVkContext->vkDevice, resource->mResourcePtr, NULL);
                } break;
            case RESOURCE_TYPE_DESC_POOL: {
                ...
                vk_DestroyDescriptorPool(mVkContext->vkDevice, resource->mResourcePtr, NULL);
                } break;
            case RESOURCE_TYPE_DESC_SET_LAYOUT: {
                ...
                vk_DestroyDescriptorSetLayout(mVkContext->vkDevice, resource->mResourcePtr, NULL);
                } break;
...
CommandBufferManager::BeginVkDrawCommandBuffer(void)
{
    ...
    VkResult err = vk_BeginCommandBuffer(mVkCommandBuffers.commandBuffer[mActiveCmdBuffer], &cmdBeginInfo);
    assert(!err);
...
CommandBufferManager::EndVkDrawCommandBuffer(void)
{
    ...
    vk_EndCommandBuffer(mVkCommandBuffers.commandBuffer[mActiveCmdBuffer]);
...
CommandBufferManager::SubmitVkDrawCommandBuffer(void)
{
    ...
    VkResult err = vk_QueueSubmit(mVkContext->vkQueue, 1, &submitInfo, mVkCommandBuffers.fence[mActiveCmdBuffer]);
    assert(!err);
...
CommandBufferManager::WaitLastSubmition(void)
{
    ...
    if(mLastSubmittedBuffer != GLOVE_NO_BUFFER_TO_WAIT) {
        ...
        err = vk_WaitForFences(mVkContext->vkDevice, 1, &mVkCommandBuffers.fence[mLastSubmittedBuffer], VK_TRUE, GLOVE_FENCE_WAIT_TIMEOUT);
        assert(!err);
        ...
        vk_ResetFences(mVkContext->vkDevice, 1, &mVkCommandBuffers.fence[mLastSubmittedBuffer]);
        assert(!err);
...
CommandBufferManager::BeginVkAuxCommandBuffer(void)
{
    ...
    VkResult err = vk_BeginCommandBuffer(mVkAuxCommandBuffer, &cmdBeginInfo);
    assert(!err);
...
CommandBufferManager::EndVkAuxCommandBuffer(void)
{
    ...
    vk_EndCommandBuffer(mVkAuxCommandBuffer);
...
CommandBufferManager::SubmitVkAuxCommandBuffer(void)
{
    ...
    VkResult err = vk_QueueSubmit(mVkContext->vkQueue, 1, &submitInfo, mVkAuxFence);
    assert(!err);
...
CommandBufferManager::WaitVkAuxCommandBuffer(void)
{
    ...
    err = vk_WaitForFences(mVkContext->vkDevice, 1, &mVkAuxFence, VK_TRUE, GLOVE_FENCE_WAIT_TIMEOUT);
    assert(!err);
    ...
    err = vk_ResetFences(mVkContext->vkDevice, 1, &mVkAuxFence);
    assert(!err);
