...
void initialize_gl4es() {
    ...
    // overides by env. variables
		...
		SHUT_LOGD("Initialising vgpu gl4es\n");
    ...
    globals4es.gl = 30;
    if(env_version) {
        snprintf(globals4es.version, 49, "vgpu %d.%d.%d\n%s gl4es wrapper %d.%d.%d", MAJOR_, MINOR_, REVISION_, env_version, MAJOR, MINOR, REVISION);
        SHUT_LOGD("Targeting OpenGL %s\n", env_version);
    } else {
        snprintf(globals4es.version, 49, "vgpu %d.%d.%d\n%d.%d gl4es wrapper %d.%d.%d", MAJOR_, MINOR_, REVISION_, globals4es.gl/10, globals4es.gl%10, MAJOR, MINOR, REVISION);
