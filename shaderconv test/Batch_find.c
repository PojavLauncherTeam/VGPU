...
int main(void){
 ...
 ptrA = (char*) malloc (sizeof(char)*countA+1);	// Allocate A memory block.
 if (ptrA == NULL)
 {
...
// printf("%s \n %d \n", ptrA, countA);	// Print the text in the A memory block and the size.
 ...
 countC = ftell (fpC);		// Getting the number of file bytes.
 rewind (fpC);
 
 ptrC = (char*) malloc (sizeof(char)*countC+1);	// Allocate A memory block.
 if (ptrC == NULL)
 {
  fputs ("Memory error", stderr); 
  exit (2);
 ...
 resultC = fread (ptrC, 1, countC, fpC);		// Read the file to the A memory block.
 if (resultC != countC)
 {
  fputs ("Reading error", stderr);
  exit (3);
...
// printf("%s \n %d \n", ptrA, countA);	// Print the text in the A memory block and the size.
 ...
 fclose (fpC);
 	...
 	int pot;
	char ** ptr_arr = (char **)malloc(500*sizeof(char*));
	int num_PTR = 0;
	pot = find("PTR_", "PTR_", &ptrC, ptr_arr, &num_PTR);        printf("Calling %d  num_PTR = %d \n", __LINE__, num_PTR);
	char * name = (char *)malloc(500);        printf("Calling %d \n", __LINE__);
	int len = 0;
	char * ptr_A;
	char * ptr_A_;
	
	char * ptr;
	for(int n=0; n<num_PTR; n++){
		ptr = ptr_arr[n];        printf("Calling %d  ptr[%d] = %p \n", __LINE__, n, ptr_arr[n]);
		ptr += 4;
		while(ptr[len] != ')'){
			len++;        //printf("Calling %d \n", __LINE__);
		}        printf("Calling %d \n", __LINE__);
		memmove(name, ptr, len);        printf("Calling %d  len = %d \n", __LINE__, len);
		name[len] = '\0';        printf("Calling %d  name = %s \n", __LINE__, name);
		ptr_A = strstr(ptrA, name);        printf("Calling %d \n", __LINE__);
		if(ptr_A){
			*ptr_A = '#';        printf("Calling %d \n", __LINE__);
		}
		ptr_A = strstr(ptrA, name);        printf("Calling %d \n", __LINE__);
		if(ptr_A){
			*ptr_A = '#';        printf("Calling %d \n", __LINE__);
